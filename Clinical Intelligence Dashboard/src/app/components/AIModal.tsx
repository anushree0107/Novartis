...
export function AIModal({ isOpen, onClose, title, content }: AIModalProps) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 animate-in fade-in duration-200">
      ...
      {/* Modal */}
      <div className="relative w-full max-w-2xl max-h-[80vh] glass-card p-6 overflow-hidden flex flex-col animate-in zoom-in-95 duration-200">
        {/* Header */}
        <div className="flex items-center justify-between mb-6 pb-4 border-b border-[#3b82f6]/20">
        ...
        {/* Content */}
        <div className="flex-1 overflow-y-auto prose prose-gray max-w-none pr-2">
          {content.split('\n').map((line, idx) => {
            if (line.startsWith('**') && line.endsWith('**')) {
              return <h3 key={idx} className="text-gray-800 mt-4 mb-2">{line.replace(/\*\*/g, '')}</h3>;
            }
            if (line.startsWith('# ')) {
              return <h2 key={idx} className="text-xl text-gray-800 mb-3 mt-5">{line.slice(2)}</h2>;
            }
            if (line.startsWith('- ')) {
        ...
        {/* Footer */}
        <div className="mt-6 pt-4 border-t border-[#3b82f6]/20 flex justify-end">
          <button
            onClick={onClose}
